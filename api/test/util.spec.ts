...
describe('Util', () => {
  it('should clean ft', () => {
    expect(Util.cleanFt('Take You There(feat.Jamie Principle)')).toBe(
      'Take You There( Jamie Principle)',
    );
    expect(Util.cleanFt('Talk About Me (Feat. Victoria Zaro)')).toBe(
      'Talk About Me ( . Victoria Zaro)',
    );
    ...
    expect(Util.cleanFt('The Right Song (feat. Natalie La Ro')).toBe(
      'The Right Song ( . Natalie La Ro',
    );
  ...
  it('should clean remix', () => {
    ...
    expect(Util.cleanRemix("Don't Kill My Vibe (Gryffin Remix)")).toBe(
      "Don't Kill My Vibe (Gryffin )",
    );
  ...
  it('should clean cut off end words', () => {
    expect(Util.cleanCutoff('Something Just Like This-Alesso Rem')).toBe(
      'Something Just Like This-Alesso',
    );
    expect(Util.cleanCutoff('one two three')).toBe('one two three');
  });
  it('should clean up garbage', () => {
    expect(Util.cleanupExtra('Something Just Like This-Alesso Rem')).toBe(
      'Something Just Like This Alesso Rem',
    );
  });
